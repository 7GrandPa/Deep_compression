...
class Mnist(object):
    ...
    def get_thresholds(self):
        ...
        for i in range(len(trainable_vars)):
            ...
            thresholds = [0.10, 0.10, 0.10,0.10, 0.16, 0.18, 0.16, 0.15]
        #print(var_means)
        #print(var_stds)
        #print(thresholds)
        return thresholds
    ...
    def prunning(self, thresholds):
        ...
        for var in trainable_variables:
            ...
            mask_temp = np.where(abs(weights_temp_np)>thresholds[trainable_variables.index(var)], 1, 0)
            #print(mask_temp)
            ...
            weights_masked = weights_temp_np*mask_temp
            #print(weights_masked)
            self.sess.run(tf.assign(var, weights_masked))
    ...
    def compress_parameters(self):
        ...
        var_csr_uint8 = []
        var_csr_deq = []
        # used to store every layers shape. 
        min_max_pairs = []
        ...
        for ele in trainable_var1_array:
            ...
            #print('csr_data' + str(type(csr_data)))
            ...
            if csr_data != []:
                ...
                diff = max_v - min_v
                # quantize the data
                uint8_csr_data = np.array((csr_data - min_v) / diff * 256, np.uint8)
                min_max_pairs.append((min_v, max_v))

                # dq data
                dq_csr_data = uint8_csr_data * (diff/256) + min_v
                #print('dq_csr_data' + str(type(dq_csr_data)))
            else:
                min_max_pairs.append((0, 0))
                uint8_csr_data = []
                dq_csr_data = []
            var_csr_uint8.append([csr_indptr, csr_indices, uint8_csr_data])
            var_csr_deq.append([csr_indptr, csr_indices, dq_csr_data])
        '''
        data type: 
        ...
        np.save('var_csr_shape_q.npy', [var_csr_uint8, var_shapes, min_max_pairs])
        np.save('var_csr_shape_dq.npy', [var_csr_deq, var_shapes])
...
def main():
    ...
    mnist.inference('var_csr_shape_dq.npy')
